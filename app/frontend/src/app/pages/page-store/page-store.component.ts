...
export class PageStoreComponent implements OnInit {
  ...
  protected readonly plugins$: Observable<Plugin[]> = this.filterInstalled$.pipe(
    combineLatestWith(this.pluginsService.plugins),
    map(([installed, plugins]: [boolean, Plugin[]]) => {
      return installed ? plugins.filter((plugin: Plugin) => plugin.installed) : plugins;
    }),
  );
